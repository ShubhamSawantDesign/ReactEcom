...
class ViewOrders extends React.Component {
    ...
    async getFilterOrders() {
        ...
        const response = await axios.get(`api/admin_auth/view_filter_contributors_orders/${params.id}/${params.fromdate}/${params.todate}?token=`+ tk);
    ...
    async getOrders() {
        ...
        const { match: { params } } = this.props
        var tk= localStorage.getItem('sa_token').replace(/['"]+/g, '');
        const response = await axios.get(`api/admin_auth/view_contributors_orders/${params.id}?token=`+ tk);
        if(response.data.orders){
            this.setState({ orders: response.data.orders,templates:response.data.templates, isLoading: false});
        }else if(response.data.error === 401){
            Swal.fire({
        ...
        if(res.data.status === 200){
        	Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href="/admin-view-contributor-orders/"+params.id;
        	...
        	this.props.history.push("/admin-view-contributor-orders"/+params.id);
        }
    }
    render(){
        const { match: { params } } = this.props;
                                        ...
                                        <div className="card card-primary">
                                            <div className="card-body row">
                                                ...
                                                <div className="form-group col-md-4 mt-2"><br/>
                                                    <a href={'/admin-view-contributor-orders/'+params.id+'/'+this.state.fromdate+'/'+this.state.todate} type="submit" className="btn btn-primary ml-2"><i className="fa fa-check"></i> submit</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <table id="example" className="table table-bordered table-striped table-responsive">
                                                    ...
                                                    {this.state.orders.map(
                                                        ...
                                                        <tr key={index}>
                                                            ...
                                                            <td>{order.name}</td>
                                                            <td className="text-center">{order.payment_id}</td>
                                                            ...
                                                            <td>
                                                                ...
                                                                <Link onClick={this.changeStatus.bind(this,order.id)} className="btn btn-success btn-sm ml-1"><i className="fas fa-bank" title="change status paid or unpaid" /></Link>
                                                        }
                                                            </td>
                                                        </tr>
                                                       )}
