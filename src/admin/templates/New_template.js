...
class New_template extends React.Component {
    constructor() {
        super();
        this.state = {
            data: [],
    ...
    async getfilterTemplates() {
        ...
        const response = await axios.get(`api/admin_auth/new_filter_template/${params.fromdate}/${params.todate}?token=`+ tk);
    ...
    async getTemplates() {
        this.setState({ isLoading: true });
        var tk= localStorage.getItem('sa_token').replace(/['"]+/g, '');
        const response = await axios.get('api/admin_auth/view_new_template?token='+ tk);
        if(response.data.templates){
            this.setState({ templates: response.data.templates, isLoading: false});
        }else if(response.data.error === 401){
            Swal.fire({
    ...
    async deleteTemplate(id){
        await Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                this.setState({id:id});
                const response = await axios.post('api/admin_auth/delete_template?token='+ tk,this.state);
                            ...
                            <div className="container-fluid">
                                <div className="row mb-2">
                                    <div className="col-sm-6">
                                        <h1>Approve & Update Digital Assets</h1>
                                    </div>
                                    <div className="col-sm-6">
                                        ...
                                        <div className="card card-primary">
                                            <div className="card-body row">
                                                ...
                                                <div className="form-group col-md-4 mt-2"><br/>
                                                    <a href={'/admin-new-template/'+this.state.fromdate+'/'+this.state.todate} type="submit" className="btn btn-primary ml-2"><i className="fa fa-check"></i> submit</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                        ...
                                                        {this.state.templates.map(
                                                            ...
                                                            <tr key={index}>
                                                                ...
                                                                <td className="text-center">
                                                                    <a href={'/admin-edit_template_'+templates.id} className="btn btn-success btn-sm"><i className="fas fa-pencil-square-o" title="Edit"></i></a>
                                                                    <Link to={'/admin-new-template'} className="btn btn-danger ml-2"   onClick={(e) => {
                                                                this.setState({ id:templates.id })
                                                                // this.deleteTemplate();
                                                            } }><i className="fas fa-trash" title="Delete" onClick={(e) => this.deleteTemplate(templates.id)} style={{fontSize:'12px !important'}}></i></Link>
                                                                </td>
                                                            </tr>
                                                            )}    
                                                    </tbody>
