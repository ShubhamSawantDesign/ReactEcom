...
class puchaseReport extends React.Component {
   constructor() {
        super();
        this.state = {
            data: [],
    ...
    async getOrders() {
        ...
        const response = await axios.get('api/vendor_auth/purchase_report?token='+ tk);
    ...
    async getFilterOrders() {
        ...
        var tk= localStorage.getItem('ve_token').replace(/['"]+/g, '');
        const { match: { params } } = this.props;
        const response = await axios.get(`api/vendor_auth/purchase_filter_report/${params.fromdate}/${params.todate}?token=`+ tk);
        if(response.data.orders){
                                ...
                                <div className="row mb-2">
                                    ...
                                    <div className="col-sm-6">
                                        <ol className="breadcrumb float-sm-right">
                                            ...
                                            <li className="breadcrumb-item active">View Purchase Reports</li>
                                        </ol>
                                    </div>
                                </div>
                                <div className="row">
                                        ...
                                        <div className="card card-primary">
                                            <div className="card-body row">
                                                ...
                                                <div className="form-group col-md-4 mt-2"><br/>
                                                    <a href={'/admin-contributer_purchase_report/'+this.state.fromdate+'/'+this.state.todate} type="submit" className="btn btn-primary ml-2"><i className="fa fa-check"></i> submit</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <table id="example" className="table table-bordered table-striped">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th className="text-center">Dig. Assets Name</th>
                                                            ...
                                                            <th className="text-center">Your Share Amount</th>
                                                    ...
                                                    {this.state.orders.map(
                                                        ...
                                                        <tr key={index}>
                                                            ...
                                                            <td className="text-center">₹{order.product_price}</td>
                                                            <td className="text-center">{order.discount}%</td>
                                                            <td className="text-center">₹{order.product_price}</td>
                                                            <td className="text-center">₹{parseFloat(order.product_price/100*JSON.parse(localStorage.getItem('ve_user')).ratio).toFixed(2)}</td>
                                                            <td className="text-center">₹{parseFloat(order.product_price - parseFloat(order.product_price/100*JSON.parse(localStorage.getItem('ve_user')).ratio).toFixed(2)).toFixed(2)}</td>
